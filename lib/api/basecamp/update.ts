...
const toListData = (data: BasecampCardUpdate): BasecampListUpdate => {
  return {
    content: data.title,
...
const updateCard = async (info: RequestCardUpdate) => {
  ...
  const basecampURL =
    ...
    (info.type === "list"
      ? `todos/${info.cardID}.json`
      : `card_tables/cards/${info.cardID}.json`);

  // Fetch the API endpoint to update a new todo or card
...
const updateCardComment = async (
  ...
  const response = await fetch(
    `https://3.basecampapi.com/5395843/buckets/${projectId}/recordings/${objectId}/comments.json`,
    {
      method: "POST",
      headers: new Headers({
        Authorization: "Bearer " + bearer?.value,
...
export { updateCard, updateCardComment };
